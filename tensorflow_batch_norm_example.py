...
def forward_propagation(X, phase, layer_count = 3, hidder_neuron = [25, 12]):

    for i in range(layer_count-1):
        layer_str = 'layer' + str(i+1)
        print(layer_str, ' initializing...')
        if i == 0:
            h1 = dense_batch_relu(X, hidder_neuron[i], phase, layer_str)
        else:
            h1 = dense_batch_relu(h1, hidder_neuron[i], phase, layer_str)
...
def model(X_train, Y_train, learning_rate = 0.01, is_training=True,
    ...
    ### START CODE HERE ### (1 line)
    output,regularizer = forward_propagation(X, phase, layer_count, hidder_neuron)
    ### END CODE HERE ###
    ...
    with tf.Session() as sess:
        ...
        print ("Train RMSE:", rmse.eval({X: X_train, Y: Y_train, phase: is_training}))
